...
Config.prototype._getParsedConfig = async function (local) {
	var parser = this._getParser(local)
	return parser ? await parser.parser.get(parser.path) : null
}
 ...
 */
Config.prototype.get = async function(filterLocal, reuseBuildstamp) {
	var k
	var config = {}
	var config_main
	var config_local
	...
	try {
		if(!this.custompath) {
			config_main = await this._getParsedConfig(false)
			config_local = await this._getParsedConfig(true)
		} else {
		// If passed directly, don't look anywhere else
			config_main = await this._getCustom(this.custompath)
		}
	...
	if (!config.dist_persistent_folder) {
		config.dist_persistent_folder = config.dist_folder
	} else {
		config.dist_persistent_folder = path.resolve(this.dir + '/' + config.dist_persistent_folder)
	}
...
Config.prototype._getCustom = async function(custompath) {
	...
	return await parser.get(resolvedPath)
