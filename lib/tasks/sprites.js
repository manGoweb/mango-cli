...
module.exports = function(gulp, config) {
	...
	return function(done) {
		...
		for(var i = 0; i < config.sprites.length; i++) {
			...
			var filename = config.sprites[i].filename || 'shapes' + (config.sprites[i].name ? '-' + config.sprites[i].name : '')
			var task = gulp.src(config.sprites[i].path, { base: config.src_folder, cwd: config.dir, allowEmpty: true })
				.pipe(svgSprite({
					svg: {
						rootAttributes: {
				...
				tasks.add(task.pipe(gulp.dest(config.dist_persistent_folder)))
				})()
		}
